...
| **Companies** | List, view, update status, verify companies | `view_companies`, `edit_companies`, `verify_companies`, `suspend_companies` |
| **Jobs** | List, view, update status, bulk actions | `view_jobs`, `edit_jobs`, `delete_jobs`, `approve_jobs`, `bulk_operations` |
...
| `edit_companies` | Update company status |
...
**Query:** Optional pagination/filters (e.g. `page`, `limit`); exact shape depends on implementation.
...
### 6.2 Get company by ID

**GET** `/api/admin/companies/:id`

**Permission:** `view_companies`

**Response (200):** Single company entity. 404 if not found.

### 6.3 Update company status
...
**Query:** Optional (e.g. page, limit, status, companyId).
...
**Response (200):** Single job entity. 404 if not found.

### 7.3 Update job status
