...
Each item in `applications` should include at least the fields used by the recruiter UI (see **docs/RECRUITER_APPLICATION_UPDATE_API.md** and **lib/api-mappers.ts** `mapApplication`). At minimum: `id`, `recruiter_id`, `candidate_id`, `job_role_id`, `portal`, `assigned_date`, `call_date`, `call_status`, `interested_status`, `not_interested_remark`, `interview_scheduled`, `interview_date`, `turnup`, `interview_status`, `selection_status`, `joining_status`, `joining_date`, `backout_date`, `backout_reason`, `hiring_manager_feedback`, `followup_date`, `notes`, `created_at`, `updated_at`, and nested `candidate`, `job_role` (with `company`), `recruiter` where applicable.
...
- **GET** `/api/recruiter/applications` with query params: **`page`**, **`limit`**, and optional filters (`recruiter_id`, `job_role_id`, etc.) and **`sort_by`**, **`sort_order`**.
- **Response:** JSON object with **`applications`** (array), **`total`** (number), **`page`** (number), **`limit`** (number), and optionally **`total_pages`**.
- **Sorting:** Support at least `sort_by=created_at` and `sort_order=asc|desc`; additional sort fields (e.g. `call_date`, `assigned_date`) improve UX.
