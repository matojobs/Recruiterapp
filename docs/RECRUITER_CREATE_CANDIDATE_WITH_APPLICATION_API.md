...
### 2.1 `candidate` (CreateCandidateDto)
...
### 2.2 `application` (CreateApplicationDto)
...
- **Optional**
  ...
  - `call_status` (string) – one of the 9 call status options (see **frontend call status constants**).
...
Exact shape should match the existing recruiter application response (used in Candidates / Sourcing pages).

---

## 4. Frontend Changes

### 4.1 Old flow (to be replaced)
